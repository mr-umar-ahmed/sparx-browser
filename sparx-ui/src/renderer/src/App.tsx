...
import {
  ...
  Clock, Globe, Command, Moon, Sun, Library, Bug, Terminal, Brain, EyeOff, PauseCircle, Bot, Wand2,
...
const StatusDot = ({ isPrivacy }: { isPrivacy?: boolean }): ReactElement => (
  <span className="relative flex h-2 w-2">
    <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isPrivacy ? 'bg-amber-400' : 'bg-emerald-400'}`} />
    <span className={`relative inline-flex rounded-full h-2 w-2 ${isPrivacy ? 'bg-amber-500' : 'bg-emerald-500'}`} />
  </span>
);

const TypingIndicator = ({ color }: { color: string }): ReactElement => (<div className="flex items-center gap-1 px-4 py-3">{[0, 1, 2].map(i => (<motion.span key={i} className="block w-1.5 h-1.5 rounded-full" style={{ backgroundColor: color }} animate={{ y: [0, -5, 0] }} transition={{ duration: 0.7, repeat: Infinity, delay: i * 0.15 }} />))}</div>);
...
export default function App(): ReactElement {
  ...
  const T = useMemo(() => {
    if (isDark) {
      return {
        bg: '#0e0e11', surface: '#18181c', surfaceHover: '#222228', border: '#2a2a35', borderMuted: '#1f1f28', text: '#e8e8f0', textMuted: '#6b6b80', textDim: '#3d3d50',
        accent: isPrivacyMode ? '#f59e0b' : '#c084fc',
        accentDim: isPrivacyMode ? 'rgba(245, 158, 11, 0.12)' : 'rgba(192, 132, 252, 0.12)',
        userBubble: isPrivacyMode ? 'linear-gradient(135deg, #d97706 0%, #92400e 100%)' : 'linear-gradient(135deg, #7c3aed 0%, #5b21b6 100%)',
        aiBubble: '#1e1e26', aiBubbleBorder: '#2a2a38', inputBg: '#131318', urlBg: '#131318', 
        urlFocusBorder: isPrivacyMode ? '#f59e0b' : '#7c3aed', 
        sidebarBg: '#0c0c10', panelBg: '#111116', shadow: '0 25px 60px rgba(0,0,0,0.7)', shadowSm: '0 4px 20px rgba(0,0,0,0.4)',
      };
    } else {
      return {
        bg: '#f5f5f7', surface: '#ffffff', surfaceHover: '#f8f8fc', border: '#e2e2ea', borderMuted: '#ebebf2', text: '#111118', textMuted: '#6b6b80', textDim: '#c0c0cc',
        accent: isPrivacyMode ? '#d97706' : '#7c3aed',
        accentDim: isPrivacyMode ? 'rgba(217, 119, 6, 0.08)' : 'rgba(124, 58, 237, 0.08)',
        userBubble: isPrivacyMode ? 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' : 'linear-gradient(135deg, #7c3aed 0%, #5b21b6 100%)',
        aiBubble: '#f8f8fc', aiBubbleBorder: '#e2e2ea', inputBg: '#f2f2f6', urlBg: '#f2f2f6', 
        urlFocusBorder: isPrivacyMode ? '#d97706' : '#7c3aed', 
        sidebarBg: '#ededf2', panelBg: '#f8f8fc', shadow: '0 25px 60px rgba(0,0,0,0.12)', shadowSm: '0 4px 20px rgba(0,0,0,0.08)',
      };
    }
  }, [isDark, isPrivacyMode]);

  if (isAuthLoading) return <div className="h-screen w-full flex flex-col items-center justify-center bg-[#0e0e11] text-white"><Loader2 className="w-8 h-8 animate-spin text-cyan-400" /></div>;
  ...
  if (!user) {
    return (
      <div className="relative h-screen w-full bg-[#0e0e11] overflow-hidden flex items-center justify-center font-sans antialiased text-white">
        <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.3, 0.1] }} transition={{ duration: 25, repeat: Infinity, ease: "easeInOut" }} className="absolute -top-[20%] -left-[10%] w-[60vw] h-[60vw] rounded-full blur-[140px]" style={{ background: `radial-gradient(circle, rgba(34, 211, 238, 1) 0%, rgba(16,185,129,0) 70%)` }} />
        <motion.div animate={{ scale: [1, 1.4, 1], opacity: [0.05, 0.2, 0.05] }} transition={{ duration: 30, repeat: Infinity, ease: "easeInOut", delay: 2 }} className="absolute top-[30%] -right-[15%] w-[50vw] h-[50vw] rounded-full blur-[120px]" style={{ background: `radial-gradient(circle, rgba(139, 92, 246, 1) 0%, rgba(245,158,11,0) 70%)` }} />
        
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="z-10 w-full max-w-md p-8 rounded-2xl backdrop-blur-3xl bg-white/5 border border-cyan-400/20 shadow-[0_0_40px_rgba(0,0,0,0.5)] flex flex-col items-center">
          <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-6 shadow-inner cursor-pointer" style={{ background: "linear-gradient(135deg, rgba(34,211,238,1) 0%, rgba(16,185,129,1) 50%, rgba(139,92,246,1) 100%)" }}><Sparkles className="text-white w-8 h-8" /></div>
  ...
  return (
    <div style={{ ...cssVars, background: T.bg, color: T.text, fontFamily: '"DM Sans", system-ui, sans-serif' }} className="h-screen w-full overflow-hidden flex flex-col select-none">
      ...
      <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}` }} className="h-12 flex items-center px-3 gap-2 shrink-0">
        ...
        <button onClick={() => handleNavigate('https://www.google.com')} style={{ color: T.textMuted, borderRadius: 6 }} className="w-8 h-8 flex items-center justify-center hover:bg-white/8 transition-colors"><Home className="w-4 h-4" /></button>
        ...
        <div className="flex-1 flex items-center gap-2 h-8 px-3 rounded-lg transition-all" style={{ background: T.urlBg, border: `1px solid ${isUrlFocused ? T.urlFocusBorder : T.border}`, boxShadow: isUrlFocused ? `0 0 0 3px ${T.accentDim}` : 'none', }}>
          <div className="flex items-center gap-1.5 shrink-0">
            <span title={isPrivacyMode ? "Privacy Mode Active" : ""}>
        ...
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsChatOpen(p => !p)} style={{ background: isChatOpen ? (isPrivacyMode ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'linear-gradient(135deg, #7c3aed, #a855f7)') : T.accentDim, color: isChatOpen ? '#fff' : T.accent, borderRadius: 8, border: `1px solid ${isChatOpen ? 'transparent' : T.border}`, }} className="h-8 px-3 flex items-center gap-2 text-xs font-semibold transition-all"><Sparkles className="w-3.5 h-3.5" />Sparx</motion.button>
        
        <div className="w-8 h-8 flex items-center justify-center transition-colors relative group" style={{ color: cloudStatus === 'synced' ? '#10b981' : cloudStatus === 'syncing' ? '#fbbf24' : cloudStatus === 'paused' ? '#f59e0b' : '#ef4444' }}>
          ...
          <div className="absolute top-10 right-0 bg-black/80 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">{cloudStatus === 'synced' ? 'Data synced to Sparx Cloud' : cloudStatus === 'syncing' ? 'Syncing...' : cloudStatus === 'paused' ? 'Cloud Sync Paused (Privacy Mode)' : 'Cloud sync disconnected'}</div>
        </div>
          ...
          {isChatOpen && (
            <motion.aside initial={{ width: 0, opacity: 0 }} animate={{ width: 400, opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ type: 'spring', stiffness: 320, damping: 30 }} style={{ background: T.panelBg, borderLeft: `1px solid ${T.border}` }} className="h-full flex flex-col overflow-hidden shrink-0">
              <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: '0 16px' }} className="h-12 flex items-center gap-3 shrink-0">
                <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: isPrivacyMode ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'linear-gradient(135deg, #7c3aed, #a855f7)' }}><Sparkles className="w-3.5 h-3.5 text-white" /></div>
              ...
              {activePanel === 'chat' && (
                <>
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
                    {chatHistory.map((msg, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.03, 0.3) }} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        {msg.role === 'ai' && <div className="w-6 h-6 rounded-md flex items-center justify-center shrink-0 mt-1 mr-2" style={{ background: isPrivacyMode ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'linear-gradient(135deg, #7c3aed, #a855f7)' }}><Sparkles className="w-3 h-3 text-white" /></div>}
                        <div className="max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-relaxed" style={msg.role === 'user' ? { background: T.userBubble, color: '#fff', borderRadius: '16px 4px 16px 16px', } : { background: T.aiBubble, border: `1px solid ${T.aiBubbleBorder}`, color: T.text, borderRadius: '4px 16px 16px 16px', }}>
                          {msg.role === 'user' ? <p className="whitespace-pre-wrap" style={{ fontSize: 13 }}>{msg.content}</p> : msg.content === '' ? <TypingIndicator color={isPrivacyMode ? '#f59e0b' : '#c084fc'} /> : (
                            <div style={{ fontSize: 13 }}>
                              ...
                              {msg.content && (
                                <div className="flex items-center gap-3 mt-3 pt-2 border-t border-white/5">
                                  <button onClick={() => copyToClipboard(msg.content)} className="flex items-center gap-1.5 transition-colors hover:text-white" style={{ color: T.textMuted, fontSize: 11 }}><Copy className="w-3 h-3" /> Copy</button>
                                  <button onClick={() => handleSaveToWorkspace(msg.content)} className="flex items-center gap-1.5 transition-colors" style={{ color: T.textMuted, fontSize: 11 }}><Library className="w-3 h-3" /> Save to Workspace</button>
                                </div>
                              )}
                            </div>
                          )}
                  ...
                  <div style={{ borderTop: `1px solid ${T.border}`, padding: '12px 14px' }} className="shrink-0 relative">
                    <AnimatePresence>
                      {currentMessage.startsWith('/') && filteredSlashCommands.length > 0 && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-full left-4 right-4 mb-2 rounded-xl overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.5)] border z-50" style={{ background: T.surface, borderColor: T.border }}>
                          ...
                          {filteredSlashCommands.map((sc, i) => (
                            <button key={i} onClick={() => { setCurrentMessage(''); handleSendMessage(sc.text); }} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-white/5" style={{ color: T.text, borderBottom: i < filteredSlashCommands.length - 1 ? `1px solid ${T.border}` : 'none' }}>
                    ...
                    <div className="relative flex items-end gap-2 rounded-xl px-3 py-2 z-10" style={{ background: T.inputBg, border: `1px solid ${T.border}` }}>
                      <input type="file" ref={fileInputRef} onChange={(e) => handleFileUpload(e.target.files?.[0])} accept=".pdf" className="hidden" />
                      <button onClick={() => fileInputRef.current?.click()} disabled={isTyping || isUploading} style={{ color: T.textMuted }} className="p-1 transition-colors mt-1 hover:text-white"><Paperclip className="w-4 h-4" /></button>
                      ...
                      <button onClick={() => handleSendMessage()} disabled={isTyping || isUploading || !currentMessage.trim()} className="p-2 rounded-lg transition-all disabled:opacity-40 hover:scale-105 active:scale-95 shrink-0" style={{ background: isPrivacyMode ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'linear-gradient(135deg, #7c3aed, #a855f7)', color: '#fff' }}><ArrowRight className="w-3.5 h-3.5" /></button>
                    </div>
              ...
              {activePanel === 'workspace' && (
                <div className="flex-1 overflow-y-auto p-4 bg-black/10">
                  ...
                  {safeNotes.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-40 text-center px-4 border border-dashed rounded-xl" style={{ borderColor: T.border }}>
                  ...
                  <div className="space-y-3">
                    {safeNotes.map(note => (
                      <motion.div key={note.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl border relative group" style={{ background: T.surface, borderColor: T.border }}>
                        <button onClick={() => { if(setNotes) setNotes(p => { const arr = Array.isArray(p) ? p : []; return arr.filter(n => n.id !== note.id); })}} className="absolute top-3 right-3 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500/20 text-red-400"><Trash2 className="w-4 h-4" /></button>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -20 }} transition={{ type: 'spring', stiffness: 400, damping: 28 }} className="fixed top-[20%] left-1/2 -translate-x-1/2 z-50 w-[600px] rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.border}`, boxShadow: T.shadow }}>
              <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: `1px solid ${T.border}` }}>
                {commandQuery.toLowerCase().startsWith('open ') ? <Globe className="w-4 h-4 shrink-0 text-blue-400" /> : commandQuery.toLowerCase().startsWith('search ') ? <Search className="w-4 h-4 shrink-0 text-green-400" /> : <Command className="w-4 h-4 shrink-0" style={{ color: T.accent }} />}
              ...
              <div className="py-2 max-h-[360px] overflow-y-auto">
                ...
                {isDeveloperMode && (commandQuery === '' || 'debug'.includes(commandQuery.toLowerCase())) && (
                  <button onClick={() => executeCommand('debug')} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Bug className="w-4 h-4 shrink-0 text-red-400" /><span style={{ color: T.text, fontSize: 13 }}>Debug current page code</span>
                  </button>
                )}
                {isDeveloperMode && (commandQuery === '' || 'refactor'.includes(commandQuery.toLowerCase())) && (
                  <button onClick={() => executeCommand('refactor')} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Terminal className="w-4 h-4 shrink-0 text-green-400" /><span style={{ color: T.text, fontSize: 13 }}>Refactor current page code</span>
                  </button>
                )}
                ...
                {(commandQuery === '' || 'agent'.includes(commandQuery.toLowerCase())) && (
                  <button onClick={() => executeCommand('agent ')} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Bot className="w-4 h-4 shrink-0 text-blue-400" /><span style={{ color: T.text, fontSize: 13 }}>Launch Autonomous Agent...</span>
                  </button>
                )}
                {(commandQuery === '' || 'extract'.includes(commandQuery.toLowerCase()) || 'note'.includes(commandQuery.toLowerCase())) && (
                  <button onClick={() => executeCommand('auto note')} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Wand2 className="w-4 h-4 shrink-0 text-yellow-400" /><span style={{ color: T.text, fontSize: 13 }}>Auto-Extract page to Workspace</span>
                  </button>
                )}
                {(commandQuery === '' || 'workspace'.includes(commandQuery.toLowerCase())) && (
                  <button onClick={() => executeCommand('workspace')} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Library className="w-4 h-4 shrink-0 text-purple-400" /><span style={{ color: T.text, fontSize: 13 }}>Open Knowledge Workspace</span>
                  </button>
                )}
                {(commandQuery === '' || 'compare'.includes(commandQuery.toLowerCase())) && (
                  <button onClick={() => executeCommand('compare')} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Columns className="w-4 h-4 shrink-0 text-pink-400" /><span style={{ color: T.text, fontSize: 13 }}>Compare all open tabs</span>
                  </button>
                )}
                ...
                {commandQuery.toLowerCase().startsWith('open ') && commandQuery.length > 5 && (
                  <button onClick={() => executeCommand(commandQuery)} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Globe className="w-4 h-4 shrink-0 text-blue-400" /><span style={{ color: T.text, fontSize: 13 }}>Go to <strong>{commandQuery.replace('open ', '')}.com</strong></span>
                  </button>
                )}
                {commandQuery.toLowerCase().startsWith('search ') && commandQuery.length > 7 && (
                  <button onClick={() => executeCommand(commandQuery)} className="w-full flex items-center gap-3 px-5 py-3 transition-colors text-left suggestion-item" onMouseEnter={e => (e.currentTarget.style.background = T.surfaceHover)} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <Search className="w-4 h-4 shrink-0 text-green-400" /><span style={{ color: T.text, fontSize: 13 }}>Search Google for <strong>"{commandQuery.replace('search ', '')}"</strong></span>
                  </button>
                )}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: 'spring', stiffness: 400, damping: 28 }} className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[480px] rounded-2xl p-6 flex flex-col gap-6" style={{ background: T.surface, border: `1px solid ${T.border}`, boxShadow: T.shadow }}>
              ...
              <div className="flex flex-col gap-3 p-4 rounded-xl border" style={{ borderColor: T.border, background: T.surfaceHover }}>
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => { logout(); setIsSettingsOpen(false); }} className="px-3 py-1.5 rounded-lg text-xs font-medium border border-red-500/30 text-red-400 hover:bg-red-500/10 transition-colors">Sign Out</button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border" style={{ borderColor: T.border, background: isPrivacyMode ? 'rgba(245, 158, 11, 0.05)' : T.surfaceHover }}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isPrivacyMode ? 'bg-amber-500/20 text-amber-500' : 'bg-emerald-500/20 text-emerald-500'}`}>
                    {isPrivacyMode ? <EyeOff className="w-5 h-5" /> : <Shield className="w-5 h-5" />}
                  </div>
                  <div className="flex-1">
                    <div className="font-medium text-sm">Privacy Shield (Incognito)</div>
                ...
                <button onClick={() => setIsPrivacyMode(!isPrivacyMode)} className={`w-12 h-6 rounded-full relative transition-colors ${isPrivacyMode ? 'bg-amber-500' : 'bg-gray-600'}`}>
                  <motion.div layout className="w-4 h-4 bg-white rounded-full absolute top-1" style={{ left: isPrivacyMode ? '28px' : '4px' }} />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border" style={{ borderColor: T.border, background: isDeveloperMode ? 'rgba(168, 85, 247, 0.05)' : T.surfaceHover }}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isDeveloperMode ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-500/20 text-gray-400'}`}>
                ...
                <button onClick={() => setIsDeveloperMode(!isDeveloperMode)} className={`w-12 h-6 rounded-full relative transition-colors ${isDeveloperMode ? 'bg-purple-500' : 'bg-gray-600'}`}>
                  <motion.div layout className="w-4 h-4 bg-white rounded-full absolute top-1" style={{ left: isDeveloperMode ? '28px' : '4px' }} />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-2 pt-2 border-t" style={{ borderColor: T.border }}>
                ...
                <button onClick={onWipeMemoryClick} className="w-full py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-colors">
